...
\[
	\int_0^1 \frac{1}{\log x}\,dx = \int_0^\frac{1}{2} \frac{1}{\log x}\,dx  + \int_\frac{1}{2}^1 \frac{1}{\log x}\,dx
\]
נציב $y= \log x$
\begin{gather*}
	y = \log x \\
	...
	x = \frac{1}{2} \implies y = -\log{2} \\
	x \to 1^- \implies y \to 0^-
\end{gather*}
ואז
\[
	\int_0^\frac{1}{2} \frac{1}{\log x}\,dx  + \int_\frac{1}{2}^1 \frac{1}{\log x}\,dx = 	\int_{-\infty}^{-\log 2} \frac{e^y}{y}\,dy  + \int_{-\log 2}^0 \frac{e^y}{y} \,dy
\]
לכל $y\in(-\infty,-\log 2]$ $\left|y\right|>1$, לכן $\frac{e^y}{\left|y\right|} < e^y$, נשתמש במבחן ההשוואה\\
נבדוק אם $\int_{-\infty}^{-\log 2} e^y \, dy$ מתכנס
\[
    \int_{-\infty}^{-\log 2} e^y \, dy  = \lim_{c\to\infty} \int_{-c}^{-\log 2} e^y \, dy \underset{N-L}{=} \lim_{c\to\infty} e^y \bigg{|}^{-\log 2}_{-c}  = \lim_{c\to\infty} e^{-\log 2} - \underbrace{e^{-c}}_{\to 0} = \frac{1}{2}
\]
...
\]
האינטגרל $\int^0_{-\log 2} \frac{1}{y} \, dy$ מתבדר, וקטן מ-$\frac{e^y}{y}$, ולכן ממבחן ההשוואה הגבולי $\frac{e^y}{y}$ מתבדר גם הוא.\\
