...
מכיוון ש-$P_W$ היא ההטלה האורתוגונלית על $W$  מתקיים $ P_W (v) = P_(W)(w+u) underset(=, P_W" לינאריות ") P_(W)(w)+P_(W)(u) = P_(W)(w) = w $
...
לכן, $v = w/norm(w)^2 + u$ עבור$u in "Span"(w)^perp$\  מכאן ש$ v
...
לפי מה שהראינו בשאלה $)3($, הוקטור מהנורמה המינימלית של $ {v in V|phi(v) =1} $ כאשר $forall v: phi(v) = ip(v, w)$ הוא $w/norm(w^2)$\
נחשב את $norm(w)^2$
$
  norm(w)^2 = ip(w, w) = integral^1_(-1) 15/4 x^2 + 3/2 x - 3/4 = lr((5x^3)/4 + (3x^2)/4 - (3x)/4 |)^1_(-1) \
  = 5/4 + 3/4 - 3/4 - (-5/4 + 3/4 +3/4) =1
$
לכן $w/norm(w)^2 = w$, ולכן, כפי שהראינו בשאלה $)3($, הוקטור  עם הנורמה המינימלית המקיים $p(1)=1$ הוא
$
  15/4 x^2 + 3/2 x - 3/4
$
